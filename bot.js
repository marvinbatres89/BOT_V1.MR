...
import {
  botEngine
} from "./bot-engine.js?v=FIX14-0-DIRECTION-MEMORY";
...
const BOT_VERSION =
  "FIX14.0-DIRECTION-MEMORY";


const BOT_BUILD =
  "DIRECTION-MEMORY-STRATEGY-AUDIT";
...
function renderTimingDireccion(resumen) {
  if (!resumen) return;
  const pintar = (el, data) => {
    if (!el) return;
    if (!data || !data.total) { el.textContent = "SIN EVIDENCIA"; return; }
    const mejor = data.mejor;
    const mejorTxt = mejor ? ` · mejor ${mejor.timingBucketMs > 0 ? "+" : ""}${mejor.timingBucketMs}ms (${mejor.accuracy}% / ${mejor.total})` : "";
    el.textContent = `${data.ganadas}G/${data.perdidas}P · ${data.accuracy ?? "--"}%${mejorTxt}`;
  };
  pintar(UI.timingEvenResumen, resumen.EVEN);
  pintar(UI.timingOddResumen, resumen.ODD);
  if (UI.ajusteEvenSelect && resumen.configuracion?.EVEN !== undefined) UI.ajusteEvenSelect.value = String(resumen.configuracion.EVEN);
  if (UI.ajusteOddSelect && resumen.configuracion?.ODD !== undefined) UI.ajusteOddSelect.value = String(resumen.configuracion.ODD);
  if (UI.timingDireccionEstado) UI.timingDireccionEstado.textContent = `AUTOMÁTICO DEMO · EVEN ${Number(resumen.configuracion?.EVEN||0)>0?"+":""}${resumen.configuracion?.EVEN ?? 0} ms · ODD ${Number(resumen.configuracion?.ODD||0)>0?"+":""}${resumen.configuracion?.ODD ?? 0} ms`;
}
...
registrarActividad(
  "FIX14.0 DIRECTION MEMORY + STRATEGY AUDIT activa.",
  "correcto"
);
