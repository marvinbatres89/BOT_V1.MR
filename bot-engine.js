/* ==========================================
   BOT V1 MR
   BOT-ENGINE.JS
   FIX14.5 FAVORABLE EVIDENCE GATE + LOSS PROTECTION
...
class BotEngine {
  ...
  constructor() {
    ...
    this.modo =
      "DERIV DEMO + FIX14.5 EVIDENCE GATE";
  ...
  analizarMovimientoParidad(
    ...
    if (
      !this.memoriaMovimientoParidad ||
      typeof this.memoriaMovimientoParidad !== "object"
    ) {
      this.memoriaMovimientoParidad = {};
    }
  ...
  analizarEvidenciaFavorable(
    ...
    const agregarSoporte = (
      ...
      const muestras =
        Number(analisis?.muestras || 0);

      const accuracy =
        Number(analisis?.accuracy);
      ...
      if (favorable) {
        soportes.push({
          ...
          muestras,
          accuracy:
            this.redondear(accuracy),
          fuerte:
            muestras >=
              FAVORABLE_EVIDENCE_CONTROL.strongSingleEvidenceSamples &&
            accuracy >=
              FAVORABLE_EVIDENCE_CONTROL.strongSingleEvidenceAccuracy
        });
      }
    };
    ...
    const timingMuestras =
      Number(
        analisisTimingBuy?.muestras || 0
      );

    const timingAccuracy =
      Number(
        analisisTimingBuy?.accuracy
      );
    ...
    const confianzaValida =
      ...
      confianza >=
        FAVORABLE_EVIDENCE_CONTROL.minimumSignalConfidence;
    ...
    const pasaTrasDosPerdidas =
      ...
      calidadCombinada >=
        FAVORABLE_EVIDENCE_CONTROL
          .minimumCombinedQualityAfterTwoLosses;
  ...
  obtenerAjusteSenal(
    ...
    return this.obtenerAjusteMercado(
      senal?.mercado
    );
  ...
  registrarTimingDireccion(
    ...
    */
    ...
    entrada.sumaTimingMs +=
      timing;


    entrada.promedioTimingMs =
      this.redondear(
        entrada.sumaTimingMs /
        entrada.total
      );
  ...
  iniciar() {
    ...
    return {
      ...
      mensaje:
        "Bot iniciado FIX14.5 FAVORABLE EVIDENCE GATE + LOSS PROTECTION."
  ...
  obtenerPreparacionManualPendiente() {
    ...
    return {
      ...
      targetDisponible:
        this.numeroValido(
          item.telemetria?.targetExecutionAt
        ),
  ...
  crearTelemetria(
    ...
    return {
      ...
      evidenciaFavorableClasificacion:
        null,

      evidenciaFavorableDecision:
        null,
  ...
  async crearPreparacion(
    ...
    telemetria.proposalPreparedEpoch =
      Date.now();
  ...
  async procesarPreparacion(
    ...
    const analisisMovimientoParidad =
      this.analizarMovimientoParidad(
        senal
      );


    const analisisTimingBuy =
      this.analizarTimingBuyDecision(
        senal
      );


    const analisisRegimenRacha =
    ...
    const analisisEvidenciaFavorable =
    ...
    telemetria.patronClasificacion =
      analisisPatron
        .clasificacion;


    telemetria.patronFuerza =
      analisisPatron
        .fuerza;


    telemetria.patronDecision =
      analisisPatron
        .decision;


    telemetria.patronMuestrasPrevias =
      analisisPatron
        .muestras;


    telemetria.patronGanadasPrevias =
      analisisPatron
        .ganadas;


    telemetria.patronPerdidasPrevias =
      analisisPatron
        .perdidas;


    telemetria.patronAccuracyPrevio =
      analisisPatron
        .accuracy;
    ...
    telemetria.movimientoParidadClasificacion =
      analisisMovimientoParidad
        ?.clasificacion ??
      null;


    telemetria.movimientoParidadFuerza =
      analisisMovimientoParidad
        ?.fuerza ??
      null;


    telemetria.movimientoParidadDecision =
      analisisMovimientoParidad
        ?.decision ??
      null;


    telemetria.movimientoParidadMuestrasPrevias =
      Number(
        analisisMovimientoParidad
          ?.muestras ||
        0
    ...
    telemetria.movimientoParidadAccuracyPrevio =
      analisisMovimientoParidad
        ?.accuracy ??
      null;


    telemetria.timingBuyClasificacion =
      analisisTimingBuy
        ?.clasificacion ??
      null;


    telemetria.timingBuyDecision =
      analisisTimingBuy
        ?.decision ??
      null;


    telemetria.timingBuyMuestras =
      Number(
        analisisTimingBuy
          ?.muestras ||
        0
    ...
    telemetria.timingBuyAccuracy =
      analisisTimingBuy
        ?.accuracy ??
      null;


    telemetria.timingBuyBloqueado =
      analisisTimingBuy
        ?.bloquear ===
      true;


    telemetria.evidenciaFavorableClasificacion =
      analisisEvidenciaFavorable
        ?.clasificacion ??
      null;


    telemetria.evidenciaFavorableDecision =
      analisisEvidenciaFavorable
        ?.decision ??
      null;


    telemetria.evidenciaFavorableBloqueada =
      analisisEvidenciaFavorable
        ?.bloquear ===
      true;


    telemetria.evidenciaFavorable =
      analisisEvidenciaFavorable
        ? this.clonarSeguro(
            analisisEvidenciaFavorable
          )
